...
const eventFormSchema = z
  ...
  .superRefine((data, ctx) => {
    if (data.start_date && data.end_date && data.end_date < data.start_date) ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'End date must be on or after start date', path: ['end_date'] });
    ...
    for (const phase of EVENT_PHASES) {
      ...
      if (open && close && close < open) ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Close date must be on or after open date', path: [`${phase}_close_date`] });
    }
  });
