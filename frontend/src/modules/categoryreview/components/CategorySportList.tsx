...
import type { CategorySportGroup, CategorySubmissionStatus } from '../types';

// The submissions endpoint paginates; pull a wide page and group client-side.
...
interface CategorySportListProps {
    onSelectSport: (group: CategorySportGroup) => void;
}

export function CategorySportList({ onSelectSport }: CategorySportListProps) {
    ...
    const { data: response, isLoading, error } = useCategorySubmissions({ limit: FETCH_LIMIT });

    const submissions = response?.data ?? [];
    // Status filter narrows the underlying rows before grouping.
    const visibleSubmissions = statusFilter === 'all'
        ? submissions
        : submissions.filter((s) => (s.status ?? 'SUBMITTED') === statusFilter);
    const groups = groupCategorySubmissionsBySport(visibleSubmissions);

    const filtered = groups.filter((g) => {
        const q = search.trim().toLowerCase();
        if (!q) return true;
        return `${g.sport_name ?? ''} ${g.eventNames.join(' ')}`.toLowerCase().includes(q);
    });
    ...
    return (
        <div className="space-y-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        ...
                        <p className="text-xs leading-relaxed text-muted-foreground">{t('sportsSubmitted')}</p>
                    </div>
                </div>
            ...
            <div className="overflow-hidden rounded-lg border border-border bg-card shadow-sm">
                <SectionHeader title={t('sportRecords.title')} subtitle={t('sportRecords.subtitle')} icon={Trophy} />
                <DataTable
                    isLoading={isLoading}
                    data={pageRows}
                    rowKey={(g: CategorySportGroup) => String(g.sports_id ?? g.sport_name ?? '')}
                    onRowClick={(g) => onSelectSport(g)}
                    columns={[
                        {
                            header: t('columns.sport'),
                            accessor: (g: CategorySportGroup) => (
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="min-w-0">
                                        <p className="text-sm font-medium leading-relaxed text-foreground">{g.sport_name || '—'}</p>
                        ...
                        {
                            header: t('sportColumns.submissions'),
                            accessor: (g: CategorySportGroup) => (
                        ...
                        {
                            header: t('sportColumns.breakdown'),
                            accessor: (g: CategorySportGroup) => (
                        ...
                        },
                        {
                            ...
                            accessor: (g: CategorySportGroup) => <div className="text-sm leading-relaxed text-muted-foreground">{new Date(g.latestSubmittedAt).toLocaleDateString()}</div>,
                        },
                    ]}
                />
                {totalPages > 1 && (
