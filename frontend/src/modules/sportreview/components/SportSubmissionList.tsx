...
import type { SportSubmission, SportSubmissionStatus } from '../types';
...
interface SportSubmissionListProps {
    onSelect: (submission: SportSubmission) => void;
}

export function SportSubmissionList({ onSelect }: SportSubmissionListProps) {
    ...
    // Client-side search of the loaded set (the endpoint has no search param).
    const filtered = submissions.filter((s) => {
        const q = search.trim().toLowerCase();
        if (!q) return true;
        return `${s.org_name ?? ''} ${s.sport_name ?? ''} ${s.event_name ?? ''}`.toLowerCase().includes(q);
    });
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <select
                    value={statusFilter}
                    onChange={(e) => { setStatusFilter(e.target.value as 'all' | SportSubmissionStatus); setCurrentPage(0); }}
                    ...
                    columns={[
                        {
                            ...
                            accessor: (s: SportSubmission) => (
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-sm font-medium leading-relaxed text-foreground">{s.org_name || '—'}</p>
                                        <p className="text-xs leading-relaxed text-muted-foreground">{s.sport_name || '—'}</p>
                                    </div>
                                </div>
                            ),
                        },
                        ...
                        },
                        {
                            ...
                            accessor: (s: SportSubmission) => <div className="text-sm leading-relaxed text-muted-foreground">{new Date(s.created_at).toLocaleDateString()}</div>,
                        },
                    ]}
                />
                {totalPages > 1 && (
