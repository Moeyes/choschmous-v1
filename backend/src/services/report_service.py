...
class ReportService:
    ...
    # ── helpers ────────────────────────────────────────────────
    def _GK(self, g) -> str:
        """Normalize a gender value (enum or str) to a lowercase key."""
    ...
    async def _enforce_org_scope(self, current_user, org_id: int | None) -> int | None:
    ...
    async def totals(
        ...
        for org in orgs:
            ...
            delegate_m, delegate_f = await self._role_counts(
                event_id, org.id, LeaderRole.DELEGATE
            )
            coach_m, coach_f = await self._role_counts(
                event_id, org.id, LeaderRole.COACH
            )
    ...
    async def _leader_totals(self, event_id: int, org_id: int) -> tuple[int, int]:
        q = (
            select(
                func.count().filter(Enroll.gender == genderEnum.MALE),
                func.count().filter(Enroll.gender == genderEnum.FEMALE),
    ...
    async def name_list(self, event_id: int, current_user) -> ReportRows:
        ...
        for org in orgs:
            ...
            leaders = await self._enrolled_leaders(event_id, org.id)
            for ld in leaders:
                ld["org_name_kh"] = org.name_kh
                ld["org_name_en"] = org.name_en
    ...
    async def delegation(self, event_id: int, current_user) -> ReportRows:
        q = (
            ...
            .join(
                OrganizerRole,
                OrganizerParticipation.organizer_role_id == OrganizerRole.id,
            )
            .outerjoin(
